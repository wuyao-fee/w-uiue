...
提供了一套常用的图标集合，由svg图片和svg-icon组件实现。
...
注意：hover时图标没有变化的，使用时需要添加`stroke`属性。
    ...
    <div class="list-item" v-for="(item, index) in list" :key="item + index">
      ...
      <ul class="icon-list">
        <li v-for="(sub, i) in item.iconList" :key="sub + i">
            <span class="li-span">
                <w-svg-icon :name="sub" color="#000" size="32" stroke v-if="showStroke(sub)"></w-svg-icon>
...
| color    | 图标颜色         | string | —                              | —   |
| stroke | 图标是否描边(少数svg通过stroke绘制)           | boolean | —                              | false   |